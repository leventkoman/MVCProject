...
namespace MyEvernote.WebApp.Controllers
{
    ...
    public class NoteController : Controller
    {
        ...
        LikedMenager likedMenager=new LikedMenager();

        [Auth]
        public ActionResult Index()
        {
            var notes =noteMenager.ListQueryable().Include("Category").Include("Owner").Where(
        ...
        {
            var notes = likedMenager.ListQueryable().Include("LikedUser").Include("Note").Where(
        ...
        public ActionResult Details(int? id)
        {
            ...
            Note note = noteMenager.Find(x=>x.Id == id.Value);
            if (note == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Edit(Note note)
        {
            ...
            if (ModelState.IsValid)
            {
                Note db_note = noteMenager.Find(x => x.Id == note.Id);
                db_note.CategoryId = note.CategoryId;
                db_note.IsDraft = note.IsDraft;
                db_note.Text = note.Text;
                db_note.Title = note.Title;

                noteMenager.Update(db_note);
                noteMenager.Save();
                return RedirectToAction("Index");
            }
            ViewBag.CategoryId = new SelectList(CacheHelper.GetCategoriesFromCache(), "Id", "Title", note.CategoryId);
        ...
        {
            Note note = noteMenager.Find(x => x.Id == id);
            noteMenager.Delete(note);
            noteMenager.Save();
            return RedirectToAction("Index");
        }
        ...
        public ActionResult GetLiked(int[] ids)
        {
            ...
            {
                List<int> likedNoteIds = likedMenager.List(
        ...
        public ActionResult SetLikeState(int noteid, bool liked)
        {
            ...
            Liked like =
                likedMenager.Find(x => x.Note.Id == noteid && x.LikedUser.Id == CurrentSession.User.Id);

            Note note = noteMenager.Find(x => x.Id == noteid);

            if (like != null && liked == false)
            {
                res = likedMenager.Delete(like);
            }
            else if (like == null && liked == true)
            {
                res = likedMenager.Insert(new Liked()
                {
